...
export const Login = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    Axios.post('https://kanban-app-beryl.vercel.app/auth/login', {
      email,
      password
    }).then(response => {
  ...
  return (
    <div className="flex items-center justify-center h-screen bg-hero bg-cover">
      <form className="bg-transparent border-2 border-gray-400 p-6 rounded shadow-md w-80"
        ...
        <Link to='/forgot-password' className="mt-2 text-blue-100 hover:text-blue-400">Forgot Password?</Link>
        <div className="my-1 border-t-2 border-blue-100 "></div>

        <Link to="/signup" className="flex justify-between text-blue-100 hover:text-blue-400">
          <p>Don't have account?</p> Sign Up
        </Link>
      </form>
    </div>
